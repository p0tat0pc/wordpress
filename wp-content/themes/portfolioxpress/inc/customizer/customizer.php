<?php
/**
 * PortfolioXpress Theme Customizer
 *
 ...
 */
function portfolioxpress_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				...
				'render_callback' => 'portfolioxpress_customize_partial_blogname',
			)
		);
		$wp_customize->selective_refresh->add_partial(
			'blogdescription',
			array(
				...
				'render_callback' => 'portfolioxpress_customize_partial_blogdescription',
			)
		);
	}

	/*Get default values to set while building customizer elements*/
	$default_options = portfolioxpress_get_default_customizer_values();
	/* Header Background Color*/
	$wp_customize->add_setting(
	    'portfolioxpress_options[header_bg_color]',
	    array(
	...
	$wp_customize->add_control(
	    new WP_Customize_Color_Control(
	        ...
	        'portfolioxpress_options[header_bg_color]',
	        array(
	            'label' => __('Header Background Color', 'portfolioxpress'),
	            'section' => 'colors',
	            'type' => 'color',
        		'priority' => 1,
	...
	require_once get_template_directory() . '/inc/customizer/theme-options/front-page-about.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/front-page-service.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/front-page-contact.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/front-page-testimonial.php';
	...
	$wp_customize->add_section( 'pro__section', array(
		'title'       => '' . esc_html__( 'View PRO Version', 'portfolioxpress' ),
		'priority'    => 2,
		'description' => sprintf(
			...
			__( '<div class="upsell-container">
                    ...
                    </ul> %s </div>', 'portfolioxpress' ),
			/* translators: %1$s: The view pro URL, %2$s: The view pro link text. */
			sprintf( '<a href="%1$s" target="_blank" class="button button-primary">%2$s</a>', esc_url( portfolioxpress_get_pro_link() ), esc_html__( 'View PortfolioXpress PRO', 'portfolioxpress' ) )
		),
	) );
	$wp_customize->add_setting( 'portfolioxpress_style_view_pro_desc', array(
		'default'           => '',
		'sanitize_callback' => '__return_true',
	) );
	$wp_customize->add_control( 'portfolioxpress_style_view_pro_desc', array(
		'section' => 'pro__section',
		'type'    => 'hidden',
	) );
}
add_action( 'customize_register', 'portfolioxpress_customize_register' );
 ...
 * @return void
 */
function portfolioxpress_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function portfolioxpress_customize_partial_blogdescription() {
 ...
 */
function portfolioxpress_customize_preview_js() {
    wp_enqueue_script( 'portfolioxpress-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-preview' ), PORTFOLIOXPRESS_S_VERSION, true );

}
add_action( 'customize_preview_init', 'portfolioxpress_customize_preview_js' );
 ...
 * @since 1.0.0
 */
function portfolioxpress_customizer_control_scripts(){
    wp_enqueue_style('portfolioxpress-customizer-css', get_template_directory_uri() . '/assets/css/customizer.css');
    wp_enqueue_script('portfolioxpress-customizer-controls', get_template_directory_uri() . '/assets/js/customizer-admin.js', array('jquery', 'jquery-ui-sortable', 'customize-controls') );
}
add_action('customize_controls_enqueue_scripts', 'portfolioxpress_customizer_control_scripts', 0);

/**
 * Generate a link to the Hive Lite info page.
 */
function portfolioxpress_get_pro_link() {
	return 'https://www.themeinwp.com/theme/portfolioxpress-pro/';
}